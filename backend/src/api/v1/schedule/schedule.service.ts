...
export async function getScheduleByDate(
  ...
  return logs.map(log => {
    ...
    return {
      ...
      name: log.med.med_name,
      dosage: log.med.dosage,
      ...
      takenStatus:
        log.status === 'taken_late'
          ...
          ? 'on-time'
          : null,
      time: log.taken_at ? log.taken_at : null,
      delayMinutes: log.minutes_offset,
      image_url: log.details?.image_url,
      tip: log.details?.tip,
      in: 'Now',
      displayButton: taken ? 'Undo' : 'Take',
      displayDueText: `Due at ${log.scheduled_time}`
    };
  });
}
...
export async function takeDose(logId: string) {
  ...
  log.taken_at = now;
  log.minutes_offset = isDelayed ? diffMinutes : 0;
